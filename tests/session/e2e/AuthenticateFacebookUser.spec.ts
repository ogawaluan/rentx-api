...
describe('AuthenticateFacebookUser - E2E', () => {
  it('Should be able to login in application with Facebook', async () => {
    const fakeUserData = {
      facebookId: faker.datatype.uuid(),
      name: faker.name.findName(),
      email: faker.internet.email(),
      facebookImage: faker.image.avatar(),
    };
  ...
  it('Should not be able to login in application without facebookId', async () => {
    const fakeUserData = {
      name: faker.name.findName(),
      email: faker.internet.email(),
      facebookImage: faker.image.avatar(),
